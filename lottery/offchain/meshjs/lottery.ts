...
export const reveal1Redeemer = (secret: string) =>
  conStr(0, [{ bytes: stringToHex(secret) }]);

export const reveal2Redeemer = (secret: string) =>
  conStr(1, [{ bytes: stringToHex(secret) }]);

const timeout1Redeemer = () => conStr(2, []);
const timeout2Redeemer = () => conStr(3, []);
...
Usage:
  ...
  deno run -A lottery.ts timeout1 <wallet_2.json>
  deno run -A lottery.ts timeout2 <wallet_1.json>
